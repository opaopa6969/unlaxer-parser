...
| optional・0/1回以上・bounded repeat・separated | runtimeのみ | UBNF lowering、Option/Vec AST fieldとmapper、境界・空一致・trivia/spanのJava比較 |
...
| mapping・capture・source-preserving AST | scalarのみ生成済み | group/optional/repeat capture、異種choice、typeof/commonField/enum、ソースsnapshot寿命 |
...
当面は公開context/combinator → optional/repeatとtyped AST → token/annotationの順で生成範囲を拡げる。次に実tinyexpressionの仕様corpusと評価器、信頼されたrustcodeblock、IDE/debuggerを進める。各行を小さなPRに分け、テスト・CI・merge・子issue closeまで行う。全体issueは未対応行を残したままcloseしない。
