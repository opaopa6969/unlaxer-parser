...
Since the v3 revision, the tinyexpression language has been substantially extended with Boolean 3-level operator hierarchy, 14 math functions, ternary expressions with mandatory parentheses, and unlimited string method dot chains. The UBNF grammar notation has been extended with the `@eval` annotation for declarative evaluation specification. A Design-Gap Exploration (DGE) methodology was developed to systematically identify specification-implementation gaps, uncovering 108 gaps across 5 sessions. Error recovery design has been advanced with sync-point recovery via `@recover` annotation. LSP refactoring code actions -- including bidirectional if/ternary conversion and if-chain to match conversion -- have been implemented.
...
5. **Design-Gap Exploration (DGE)** (Section 3.9): a systematic methodology for discovering specification-implementation gaps through adversarial testing and dialog-driven analysis, demonstrated through 5 sessions that uncovered 108 gaps.
6. **`@eval` annotation for declarative evaluation** (Section 3.10): an extension to UBNF that specifies evaluation semantics directly in the grammar, further reducing hand-written evaluator code and enabling compiler-checked evaluation completeness at the grammar level.

The remainder of this paper is structured as follows. Section 2 reviews related work in parser generation, language workbenches, and IDE protocol support. Section 3 presents the system design, including the UBNF grammar notation, the generation pipeline, the four contributions, operational semantics of PropagationStopper, monadic interpretation, MatchedTokenParser, DGE methodology, and the `@eval` annotation. Section 4 describes the implementation, including the new language features (Boolean operators, math functions, ternary expressions, string method chains). Section 5 evaluates the framework using tinyexpression and a palindrome recognizer case study, presenting both performance benchmarks and development effort comparisons. Section 6 discusses error recovery design, incremental parsing for LSP, LSP refactoring code actions, limitations, threats to validity, and future work. Section 7 concludes.
...
| **unlaxer-parser** | **Yes** | **Yes** | **Yes** | **Yes** | **Yes** | **Yes** | **Designed** | **Yes** |

*Table 1: Generated artifacts by tool. Spoofax's mapper is written in Stratego, a separate DSL. Xtext's LSP support requires the Xtext runtime. MPS operates on a projectional editing paradigm distinct from text-based LSP. unlaxer's error recovery is designed (sync-point recovery via @recover annotation) with partial parse success already working; full integration is in progress. Refactoring refers to LSP code actions for structural transformations.*
...
**The `@eval` annotation** (new in v4) specifies evaluation semantics directly in the grammar:
...
The `@eval` annotation generates evaluator method bodies directly from the grammar, reducing hand-written code for common patterns such as math function dispatch. See Section 3.10 for details.
...
The `EvalAnnotation` record is new in v4, parsed by the UBNF parser and mapped to the AST alongside other annotations. The parser recognizes the `@eval(dispatch=..., methods={...})` syntax and produces a structured representation that the code generator consumes.
...
| `EvaluatorGenerator` | `XxxEvaluator.java` | Abstract class with exhaustive sealed-switch dispatch, a `DebugStrategy` interface for step-debugging hooks, and generated method bodies for `@eval`-annotated rules. |
| `LSPGenerator` | `XxxLanguageServer.java` | LSP server with completion (keywords, `@catalog` entries, `@declares` symbols), diagnostics (parse errors and `ErrorMessageParser` metadata), hover (`@doc` annotations), and code actions (refactoring operations). Includes scope-store registration when `@declares`/`@backref`/`@scopeTree` annotations are present. |
...
**Results.** Five DGE sessions were conducted during the v3-to-v4 development period. The sessions uncovered **108 gaps** spanning the following categories:

| Category | Count | Example |
|----------|-------|---------|
| Missing evaluation logic | 34 | `not()` function not implemented in P4 evaluator |
| Parser-evaluator mismatch | 22 | Ternary parsing succeeded but mapper produced wrong AST node |
| Type coercion gaps | 18 | `toNum("3.14")` returned String instead of Number |
| Error message quality | 15 | Missing operator in `3 + ` produced generic "parse failed" instead of "expected expression" |
| LSP/DAP integration | 11 | Completion did not suggest math function names |
| Test coverage | 8 | No test for `min($a, $b, $c)` variadic form |
...
**Syntax.** The `@eval` annotation supports three forms:

```ubnf
// Form 1: Dispatch by name
@eval(dispatch=name, methods={
  sin:  Math.sin(toDouble(args[0])),
  cos:  Math.cos(toDouble(args[0])),
...
### 5.6 DGE Session Results

Five DGE sessions were conducted during the v3-to-v4 development period, systematically exploring the interaction between grammar rules, parser behavior, AST construction, evaluation logic, and IDE integration. The sessions uncovered 108 gaps, of which 97 have been resolved and 11 are tracked as known limitations.
...
unlaxer-parser addresses this challenge through a multi-layered error recovery design:
...
**Layer 3: Sync-point recovery via `@recover` annotation (designed).** The `@recover` annotation designates grammar rules as sync points for error recovery:
...
When parsing of a `Statement` fails, the recovery mechanism scans forward to the next sync token (`;` or `}`), skips the erroneous input, and resumes parsing at the sync point. This is analogous to ANTLR's "panic mode" recovery but specified declaratively in the grammar.

The `@recover` annotation is designed and the parser infrastructure supports it, but full integration with the LSP server's diagnostic reporting is in progress. The current implementation reports the skipped region as a diagnostic warning, enabling the developer to see both the error location and the recovery point.

This design addresses a concern raised in earlier reviews about ANTLR's error recovery advantage. While ANTLR's built-in recovery strategies are more mature, unlaxer's approach has the advantage of being specified in the grammar (via `@recover`) rather than being a fixed runtime behavior, giving the language designer control over where and how recovery occurs.

### 6.2 Incremental Parsing for LSP
...
The incremental parsing design for unlaxer-parser is specifically targeted at the LSP server, not the runtime evaluator:
...
**Chunk-based incremental parsing:** The designed approach divides the input into chunks at comma and semicolon boundaries (natural delimiters in match expressions). When the input changes, only the affected chunk is re-parsed, and the parse results for unchanged chunks are reused. For a 470-case match expression, editing one case requires re-parsing only that case (typically 30-50 characters) rather than the entire 23KB formula.

This design is currently in prototype stage. The chunk boundary detection is implemented, and preliminary measurements show a 10-50x reduction in re-parse time for large match expressions.
...
### 6.4 LSP Refactoring Code Actions

The generated LSP server provides two refactoring code actions that demonstrate the value of the unified grammar-to-IDE pipeline:
...
### 6.5 Recognition Power of MatchedTokenParser
...
**Full `@recover` integration.** Complete integration of the `@recover` annotation with the LSP server's diagnostic reporting and the DAP server's error-state display is planned.

**Incremental parsing deployment.** The chunk-based incremental parsing design (Section 6.2) will be implemented and evaluated against production-sized formulas.
...
Additionally, the **`@eval` annotation** for declarative evaluation specification and the **Design-Gap Exploration (DGE)** methodology represent methodological contributions to DSL development practice.

Our evaluation demonstrates the practical impact. tinyexpression, a production expression evaluator processing **10^9 transactions per month** as a UDF in financial transaction processing, has been substantially extended with Boolean 3-level operator hierarchy, 14 math functions, ternary expressions, and unlimited string method dot chains. It achieves a **13x reduction** in maintainable code (from ~15,000 to ~1,170 lines) compared to a from-scratch implementation. Performance benchmarks show a **1,400x improvement** from reflection-based to sealed-switch evaluation, with the sealed-switch evaluator operating within **2.8x** of JIT-compiled bytecode. Five DGE sessions uncovered 108 specification-implementation gaps, demonstrating the value of systematic gap exploration in DSL development. LSP refactoring code actions -- including bidirectional if/ternary conversion -- leverage the unified grammar-to-AST pipeline for zero-cost structural transformations.
...
The 11 unresolved gaps are tracked as known limitations and are primarily related to advanced string operations and legacy parser fallback scenarios.
...
| Paper (EN) | Review |
|------------|--------|
| [← v3 Paper](../v3/from-grammar-to-ide.en.md) | — |
| **v4 — current** | [v4 Review](./review-dialogue-v4.ja.md) |
| [v5 Paper →](../v5/from-grammar-to-ide.en.md) | [v5 Review](../v5/review-dialogue-v5.ja.md) |
