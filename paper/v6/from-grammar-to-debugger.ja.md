...
追加の構造化診断APIでは、構文不一致と成功prefixの後の未消費入力を区別し、backend固有の最遠失敗情報も保持する。既存corpusの失敗66件で種別・主要コードポイント位置が一致し、うち未消費入力25件では期待候補も一致した。構文不一致41件の期待候補は異なる。別の8境界例のうち`neg(1e+)`はJavaが位置7、Rustが位置5を返す。指数部内部の失敗を記録する粒度の差をテストに残し、診断を等価に見せる正規化は行わない。生の診断と再現手順は追加artifactに含む。

この結果は、同じfixtureの構造的な保守チェックをJavaのsealed型とRustのenum/traitで表現できることを示す。言語非依存性やruntime・診断全体の等価性の証明ではない。またrule深さ上限がある。性能・一般的な文法網羅性は未評価である。
