...
An additional structured-diagnostic API distinguishes syntax failure from trailing input after an accepted prefix, retaining backend-native farthest-failure information. Kind and primary code-point offset agree for the 66 rejected input-stage cases in the existing corpus; expected hints also agree for the 25 trailing-input cases, but differ for all 41 syntax cases. Eight additional boundary cases include an explicit counterexample: `neg(1e+)` reports offset 7 in Java and 5 in Rust because exponent-internal failures are recorded at different granularity. The artifact retains both native diagnostics rather than normalizing away this difference.

This is evidence that the structural maintenance checks can be expressed using both Java sealed types and Rust enums/traits for this fixture. It is not a proof of target-language independence or whole-runtime/diagnostic equivalence. The runtime also imposes a rule-depth limit. Performance and general grammar coverage remain unevaluated.
