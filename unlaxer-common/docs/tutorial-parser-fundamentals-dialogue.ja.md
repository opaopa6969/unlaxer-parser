...
**先輩**: [UBNF から LSP/DAP へチュートリアル](../../unlaxer-dsl/docs/tutorial-ubnf-to-lsp-dap-dialogue.ja.md) で UBNF の詳しい使い方を解説している。そちらでは:
...
**後輩**: ContainerParser って具体的にどう使うんですか？

**先輩**: 一番わかりやすいのは ErrorMessageParser だね。実際のコードを見てみよう。
...
**後輩**: あ、ContainerParser\<String\> を継承して、get() でエラーメッセージの文字列を返すんですね。

**先輩**: そう。parse() の中身も面白い。ContainerParser の parse() は Token.empty() で空のトークンを作って、現在のトークンの子に追加する。つまり入力を一切消費せずにトークンだけ挿入する。

```java
// ContainerParser.parse() の実装:
public Parsed parse(ParseContext parseContext, TokenKind tokenKind, boolean invertMatch) {
...
**後輩**: 入力を消費しないのにトークンが増えるのか...不思議ですね。

**先輩**: そしてパース後にエラー情報を取り出すには getRangedContents() を使う。

```java
// パース結果のルートトークンからエラーメッセージを全部集める
List<RangedContent<String>> errors =
    ContainerParser.getRangedContents(rootToken, ErrorMessageParser.class);

// 各エラーには位置情報（CursorRange）とメッセージが入っている
for (RangedContent<String> error : errors) {
    System.out.println(error.getContent());  // エラーメッセージ
    System.out.println(error.getRange());    // ソース上の位置
}
```

**後輩**: つまりパースツリーをエラー情報の運び屋にしてるんですね。

**先輩**: そう！LSP の diagnostics はここから情報を取れる。パースとエラー収集が一体化してるのがポイントだ。パースツリーをトラバースするだけで、構文情報もエラー情報も一度に取得できる。

**後輩**: 別途エラーリストを管理しなくていいのは楽ですね。
...
**後輩**: ところで、伝播の制御って具体的にどういう時に必要なんですか？表だけだとピンとこなくて...

**先輩**: 具体的なシナリオで説明しよう。QuotedParser の中身を考えてみて。

```
式全体を先読み（matchOnly）でチェックしたい:
  MatchOnly(
    Chain(
      WordParser("'"),
      ZeroOrMore(contentParser),  ← ここも matchOnly になってしまう！
      WordParser("'")
    )
  )

問題: contentParser まで matchOnly になると、
      引用符の中身を実際に消費できない。

解決: AllPropagationStopper で遮断
  MatchOnly(
    Chain(
      WordParser("'"),
      AllPropagationStopper(ZeroOrMore(contentParser)),  ← consumed に戻す
      WordParser("'")
    )
  )
```

**後輩**: あー、親の matchOnly が子にまで伝播しちゃうから、途中で「ここから先は consumed に戻す」って宣言が必要なんですね。

**先輩**: その通り。AllPropagationStopper は TokenKind も invertMatch も両方リセットするから、完全に「ここから先は通常モード」にできる。部分的に制御したい場合は DoCounsumePropagationStopper（TokenKind だけリセット）や InvertMatchPropagationStopper（invertMatch だけリセット）を使い分ける。

**後輩**: 伝播を「止める」んじゃなくて「遮断して既定値に戻す」というイメージですね。
...
**後輩**: 理屈はわかるんですけど、実際のパーサーではどう使い分けるんですか？

**先輩**: 実際のパーサーでの使い分けを見せよう。Not の典型的な使い方は「ある種類のトークンを除外してマッチさせたい」場面だ。

```java
// ExclusiveNakedVariableParser での Not の使い方:
// 「型宣言じゃない識別子」をマッチさせたい
Chain(
  Not(Choice(                    // ← 「次が型宣言じゃないことを確認」
    WordParser("number"),
    WordParser("string"),
    WordParser("boolean")
  )),
  IdentifierParser               // ← 「型宣言じゃないならマッチ」
)
```

**後輩**: なるほど、Not でまず「これじゃないこと」を確認してから、本命のパーサーをマッチさせるんですね。

**先輩**: 一方 MatchOnly は「ここからこの構文が始まることを確認してから、本格的なパースに入る」という使い方が多い。

```java
// 「関数呼び出しかもしれない」ことを先に確認してからパース
Chain(
  MatchOnly(Chain(               // ← 「識別子 + ( が来ることを確認」
    IdentifierParser,
    WordParser("(")
  )),
  FunctionCallParser             // ← 「確認できたら本格的にパース」
)
```

**後輩**: MatchOnly は「前提条件の確認」、Not は「除外条件の確認」ってイメージですね。

**先輩**: いい整理だ。覚えておくと設計で迷わなくなるよ。
