...
/**
 * LSPGenerator / LSPLauncherGenerator が生成する Java ソースが
 * 実際にコンパイルできることを検証するテスト。
 */
public class LSPCompileVerificationTest {
    ...
    private static CodeGenerator.GeneratedSource astResult;
    private static CodeGenerator.GeneratedSource parserResult;
    private static CodeGenerator.GeneratedSource lspServerResult;
    private static CodeGenerator.GeneratedSource lspLauncherResult;

    @BeforeClass
    public static void setUp() {
        grammar = UBNFMapper.parse(TINYCALC_GRAMMAR).grammars().get(0);
        astResult = new ASTGenerator().generate(grammar);
        parserResult = new ParserGenerator().generate(grammar);
        lspServerResult = new LSPGenerator().generate(grammar);
        lspLauncherResult = new LSPLauncherGenerator().generate(grammar);
    }

    @Test
    public void testLanguageServerCompiles() {
        assertCompiles(astResult, parserResult, lspServerResult);
    }

    @Test
    public void testLauncherCompiles() {
        assertCompiles(astResult, parserResult, lspServerResult, lspLauncherResult);
    }
    ...
    private void assertCompiles(CodeGenerator.GeneratedSource... sources) {
        ...
        List<JavaFileObject> sourceObjects = Arrays.stream(sources)
            .map(LSPCompileVerificationTest::toJavaFileObject)
            .toList();
