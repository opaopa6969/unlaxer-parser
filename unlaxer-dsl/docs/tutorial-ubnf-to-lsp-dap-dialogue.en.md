...
```mermaid
flowchart TD
    Expr["Expression string '$a + $b * 2'"]
    Compile["compile family: expr → Java code → javac → exec"]
    AST["AST family: expr → AST → recursive eval"]
    C1["[1] compile-hand<br/>JavaCodeCalculatorV3"]
    C2["[2] compile-dsl<br/>DslJavaCodeCalculator<br/>→ P4TypedJavaCodeEmitter"]
    A3["[3] ast-hand<br/>AstNumberExpressionEvaluator<br/>(@TinyAstNode annotation-driven)"]
    A4["[4] P4-reflection<br/>GeneratedP4ValueAstEvaluator<br/>(reflection, legacy)"]
    A5["[5] P4-typed *recommended<br/>P4TypedAstEvaluator<br/>(sealed switch, fast)"]
...
classDiagram
    class TinyExpressionP4Evaluator~T~ {
        <<abstract>>
        sealed switch dispatch (generated)
    }
    class P4DefaultAstEvaluator~Object~ {
        generated (@eval default/template implementations)
...
```mermaid
flowchart TD
    U["1. User inputs expression: '$price * 1.1'"]
    E[2. AstEvaluatorCalculator is the entry point]
    P4["<b>3a. P4 AST path</b><br/>TinyExpressionP4Mapper.parse()<br/>Token tree → sealed AST<br/>P4TypedAstEvaluator.eval()"]
    Java["<b>3b. JavaCode fallback path</b><br/>JavaCodeCalculatorV3<br/>expr → Java code → javac → .class<br/>.class.evaluate(context)"]
    Ctx[4. Retrieve $price value from CalculationContext]
    Ret[5. Return calculation result]
